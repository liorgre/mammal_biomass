...
def read_data() -> pd.DataFrame:
    data_file_names = dict(
        mammal_labeled='svr_data/only_global.csv',
...
def preproc_data(data_raw: pd.DataFrame):
    ...
    data = data.assign(log_density=data.log_population - data.log_range)
    ...
    data = data.drop(['Family', 'Genus', 'Order', 'Range_m', 'RedListStatus', 'TrophicLevel', 'pop_density',
...
# Find overlap between files

# labeled = pd.read_csv('combined_dataset.csv')
# labeled = labeled[labeled.label=='Entire range'].Binomial
# labeled = np.array(labeled)
# labeled.shape
# not_labeled_df = pd.read_csv('Mammal_AI_not_global.csv')
# not_labeled = np.array(not_labeled_df.Binomial)
# # not_labeled = np.array(not_labeled)
# intersection = np.intersect1d(labeled, not_labeled)
# not_labeled_df_no_overlap = not_labeled_df[~not_labeled_df.Binomial.isin(intersection)].to_csv('Mammal_AI_not_global_no_overlap.csv')