...
#
# def gen_smooth_plot(gridded_data):
#     base_unit = 0.001
#     centeroids = gpd.GeoDataFrame(gridded_data.centroid)
#     centeroids = centeroids.assign(total_mass_Mt=gridded_data.total_mass_Mt)
#     centeroids = centeroids.rename(columns={0: "geometry"})
#     num_of_points = np.int64(np.array(gridded_data.total_mass_Mt / base_unit).round())
#     gridded_data = gridded_data.assign(num_of_points=num_of_points*base_unit)
#     series = centeroids['geometry']
#     dups_data_frame = gpd.GeoDataFrame(series.repeat(num_of_points))
#     continents = gpd.read_file('continent_shapefile/continent.shp')
#     dups_data_frame = dups_data_frame.to_crs(continents.crs)
#     # plt.rcParams["figure.figsize"] = (30, 10)
#     font = {'weight': 'normal',
#             'size': 22}
#     plt.rc('font', **font)
...
#     cmap = plt.cm.get_cmap('gist_earth').reversed()
#     # cmap = gen_custom_cmap()
#     ax = gplt.kdeplot(dups_data_frame, shade=True, cmap=cmap,
#                       clip=continents.geometry,
#                       n_levels=1000, figsize=(30, 20))
#     ax.set_title('Wild Mammal Mass')
#     base = gridded_data.plot(column='num_of_points', ax=ax, legend=True, cmap=cmap)
#     continents.plot(ax=base, fc='none', ec='grey', linewidth=0.3)
#
